...
type UserRow = RowDataPacket & { id: string; email: string; password_hash: string | null; role: "customer" | "admin"; email_verified_at: Date | null; disabled_at: Date | null; totp_enabled: number };
...
export async function POST(request: NextRequest) {
  ...
  const matches = await rows<UserRow>(
    "SELECT id, email, password_hash, role, email_verified_at, disabled_at, totp_enabled FROM users WHERE email = ? LIMIT 1",
    [email]
  );
  ...
  await issueSession(user.id, "full");
  await run(
    "INSERT INTO audit_log (actor_id, action, target_type, target_id, ip_hash) VALUES (?, 'customer.login', 'user', ?, ?)",
    [user.id, user.id, sha256(ip)]
  );
